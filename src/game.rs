...
impl Persona {
    ...
    /// 同一个 Persona 在狼人杀里的高玩档案。
    /// 这里塑造的是 5 种**典型高水平玩家**的策略风格，让 AI 在 quip / 投票 /
    /// 站警 / 跳身份等所有决策上都体现差异化的"流派"。
    pub fn werewolf_description(self) -> &'static str {
        match self {
            // 莽哥 → 悍跳激进流（高调高压型高玩）
            Persona::LooseAggressive =>
                "**悍跳激进流**。第一时间表态、抢警、爆身份。狼牌敢深水悍跳预言家 \
                 编一份查验报告压场；好牌敢领跳爆身份。靠话术密度 + 节奏压迫对手，\
                 让弱玩家跟着你走、强玩家被迫接战。错了也不复盘，下一秒继续推。\
                 喜欢用绝对句式（如『X 100% 是狼』），不留余地。",

            // 老炮 → 逻辑推理流（冷静缜密型高玩）
            Persona::TightAggressive =>
                "**逻辑推理流**。听完所有信息才开口，每句话都精准短促。会回顾每一轮：\
                 谁的查杀对得上谁的、谁的发言前后矛盾。说话像在画 logic 网——一旦\
                 把某人锁进网格就咬死不放。不靠直觉、不被气势带，下结论永远基于\
                 『如果 X 是狼那 Y/Z 应该……』的链条。话不多，每开口一次场上都要重新评估局势。",

            // 跟注站 → 节奏控场流（推票指挥型高玩）
            Persona::LooseWeak =>
                "**节奏控场流**。不抢首发也不蹭尾，听 2-3 位发言后才开口梳理观点。\
                 像主持人——常用『我觉得这把 X 最像狼，你们怎么看』主导讨论方向。\
                 关键票一定带头表态拉人。不冒险，靠把场上信息揉成『大家的共识』\
                 来推关键票，狼局也常用同样手法把怀疑导向真好人。",

            // 老抠 → 静水深流（深水观察型高玩）
            Persona::TightWeak =>
                "**静水深流**。话最少，每句都精准。喜欢做听众，最后归票时给出致命一击。\
                 投票打的是『前面所有发言信息汇总后的最优解』，不被情绪带。\
                 村民职业最像，狼也常用此风格藏深水到决赛圈。\
                 沉默不是没思考，是在等开火点——一开口就是结论。",

            // 头铁 → 反水诡道流（心机算计型高玩）
            Persona::Maniac =>
                "**反水诡道流**。擅长伪装阵营 + 制造混乱。狼牌敢反咬队友 \
                 （爆身份装好人 / 卖队友换信任）；好牌假装狼把自己投上去给\
                 场上送验。反直觉打法，但每一步都在算更深一层——追求\
                 『出乎意料但算计周到』的局面。看似乱，其实别人在第 1 层你在第 3 层。",
        }
    }
